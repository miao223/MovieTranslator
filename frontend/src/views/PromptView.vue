    ...
    <el-col :span="13">
      <el-card shadow="never" class="section">
        ...
        <el-form label-width="140px">
          <el-form-item label="转写预处理">
            <el-switch v-model="settings.prompts.refine_enabled" />
            <span class="hint">推荐开启</span>
            <div class="hint" style="margin: 4px 0 0; display: block; line-height: 1.7">
              翻译前先用同一个模型把原文整理一遍：把被语音识别切断的半句话合并成完整句子、修正同音词与漏词。<br>
              这能从源头消除「一句话被拆成两三条字幕」以及由此引发的<strong>译文整行错位</strong>问题。<br>
              整理结果会逐行校验（行号必须完整覆盖、内容不得被改写），校验不过自动沿用原始分行，不影响字幕产出。<br>
              代价：翻译前多一轮覆盖全片原文的请求（按长度自动分批），约增加 30-50% 的 token 消耗与几分钟耗时。
            </div>
          </el-form-item>
          ...
          <el-form-item label="碎行衔接">
            ...
            <span class="hint">相邻行常是半句话，要求结合上下文理解，但不得把内容挪到别的行</span>
          </el-form-item>
