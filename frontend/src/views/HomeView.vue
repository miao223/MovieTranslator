      ...
      <el-form-item v-if="mode === 'single'" label="画面翻译">
        <div style="width: 100%">
          ...
          <div class="hint" style="margin: 4px 0 0; display: block">
            （可选）翻译画面中不会被语音读出的文字（手机屏幕、纸质信件、电视画面、招牌等），译文将在对应时间点显示于屏幕左上角，固定持续 5 秒。<br>
            <strong>两种使用方式，任选其一：</strong><br>
            ① <strong>随完整翻译一起</strong>：填好时间点后照常点上方「开始翻译」——语音字幕和画面译文会一次性生成在同一个新字幕文件里，适合翻译前就知道哪些画面需要翻的情况；<br>
            ② <strong>事后单独补充</strong>：点「⚡ 仅补充到已有字幕」——跳过语音识别和全文翻译（几秒到几十秒完成），只把画面译文插入视频旁<strong>已存在</strong>的同名字幕文件（.ass 与 .srt 都在时优先 .ass，原有内容不受影响），适合翻完观看后才发现有遗漏的情况。<br>
            时间格式（用半角冒号）：<code>5</code> = 第 5 秒；<code>8:19</code> = 8 分 19 秒；<code>1:02:03</code> = 1 小时 2 分 3 秒，支持小数秒（如 <code>8:19.5</code>）。<br>
            两种方式都需要模型具备视觉能力（设置 → 翻译模型 → 视觉模型）；单条失败只记录日志，不影响正常字幕。
          </div>
        </div>
      </el-form-item>
      <el-form-item>
