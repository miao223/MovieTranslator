...
跨平台（Windows / Linux）电影字幕翻译软件。把视频或音频文件一键变成高质量双语 SRT 字幕：

**视频 / 音频 → PyAV 提取音频 → Faster Whisper 语音识别 → LLM 全局上下文翻译 → SRT**
...
- **单独的字幕文件也能翻**：手里只有一份 `.srt`（或蓝光拆出来的 `.sup`、DVD 的 `.idx`+`.sub`）时，直接选它就行——图形字幕会先 OCR 认字。产物顶掉源文件的语言后缀（`片名.en.srt` → `片名.zh.srt`），**源文件不会被改动**。
- **音频文件同样可用**：只拿到音轨（mp3 / m4a / flac…）时照常出字幕——网络传输常只传音轨，识别与翻译本就不需要画面。这类片源没有画面可用，因此不支持「合成带字幕的新视频」与「画面翻译」。
- **隐私友好**：中间文件只存本地缓存目录，每次启动自动清空。
- **网页 UI**：任务进度实时推送（分阶段进度条 + 日志），支持源语言选择、剧情简介（提升翻译准确性）、四种字幕形式（双语 / 纯译文 / 纯原文 / 双语分离成两个文件）、字幕与识别参数设置。产物一律带语言后缀（`片名.en-zh.srt` / `片名.zh.srt` / `片名.en.srt`），且**绝不覆盖片源目录里已有的文件**。
...
2. 回到首页选择视频或音频文件、源语言（可 auto）、目标语言，可选填剧情简介；
...
提供的工具：列出目录里的视频与音频文件、列音轨、发起单片翻译、发起整目录批量翻译、查询任务/批量进度、取消、取回字幕全文、取回任务日志、查询服务状态。
...
- 若在设置里把识别引擎改成「API（多模态大模型）」，音频会分段（默认 5 分钟一段）上传到你自己配置的接口——这是本软件唯一会上传音频的情况，默认关闭
